...
public class ChessMatch {
	...
	public ChessPiece PerformChessmove(ChessPosition sourceposition, ChessPosition targetposition) {
		...
		check = (testCheck(OponnentColor(currentPlayer))) ? true:false;
		
		if(testCheckMate(OponnentColor(currentPlayer))) {
			checkMate = true;
		}else {
			nextTurn();
		}
	...
	private boolean testCheck(Color color) {
		...
		List<Piece>opponentPieces = piecesOnTheBoard.stream().filter( x->((ChessPiece)x).getColor() == OponnentColor(color) ).collect(Collectors.toList());
